...
transType' :: Index -> Type Index -> F.Type Index
transType' _ (TVar n a)     = F.TVar n a
transType' _ (JClass c)     = F.JClass c
transType' i (Fun a1 a2)    = F.Fun (transType' i a1) (transType' i a2)
transType' i (Forall n f)   = F.Forall n (\a -> transType' (i + 1) $ fsubstTT i (tVar a) (f i))
...
infer' this i j (F.TApp f t)          = F.joinType ((unsafeCoerce g :: t -> F.Type t) t) where F.Forall n g  = this i j f
...
infer' this i j (F.RecordElim e l1)   = transType' i t1 where Just (_,t1) = getter i (this i j e) l1
...
transExpr' super this i j (F.RecordElim e l1)        = App c (snd (this i j e)) where Just (c, _) = getter i (super i j e) l1
...
getter i (F.And t1 t2) l
  = case getter i t2 l of
      Just (c,t) ->
        Just (lam (transType i (F.And t1 t2)) (App c . Proj 2 . var), t)
      Nothing    ->
        case getter i t1 l of
          Nothing    -> Nothing
          Just (c,t) ->
            Just (lam (transType i (F.And t1 t2)) (App c . Proj 1 . var), t)
getter i (F.Thunk t1) l
...
putter i (F.And t1 t2) l e
  = case putter i t2 l e of
      Just (c,t) ->
        Just (lam (transType i (F.And t1 t2)) (\x -> Tuple [Proj 1 (var x), App c (Proj 2 (var x))]), t)
      Nothing    ->
        case putter i t1 l e of
          Nothing    -> Nothing
          Just (c,t) ->
            Just (lam (transType i (F.And t1 t2)) (\x -> Tuple [App c (Proj 1 (var x)), Proj 2 (var x)]), t)
putter i (F.Thunk t1) l e
...
force' :: F.Expr t e -> F.Expr t e
force' e = F.App e (F.Lit S.UnitLit)
