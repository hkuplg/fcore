...
containsNext l = foldr (||) False $ map (\x -> case x of (J.BlockStmt (J.ExpStmt (J.Assign (
                                                                J.NameLhs (J.Name [J.Ident nextClass,J.Ident "next"])) J.EqualA _))) -> True
                                                         _ -> False) l
...
stackbody t =
        applyCall : whileApplyLoop "c" (J.Ident "result") (case t of C.JClass ("java.lang.Integer") -> (javaClassType "java.lang.Integer")
                                                                     _ -> objType) ++ [
...
transS this super = TS {toTS = super {
  translateM = \e -> case e of
       C.Lam s           -> local (&& False) $ translateM super e
       C.Fix t s         -> local (&& False) $ translateM super e
       C.TApp _ _        -> local (&& False) $ translateM super e
       C.If e1 e2 e3   -> translateIf (up this) (local (|| True) $ translateM (up this) e1) (translateM (up this) e2) (translateM (up this) e3)
       C.App e1 e2       -> translateApply (up this) (local (|| True) $ translateM (up this) e1) (local (|| True) $ translateM (up this) e2)
