...
module StackTransCFJava2 where
...
transS this super = TS {toTS = super {
  ...
  genApply = \f _ x jType ctempCastTyp ->
      do (tailPosition :: Bool) <- ask
         ...
         put (n+1)
         if tailPosition
           then nextApply (up this) (left $ var f) x jType
           else (whileApply (up this) True (left $ var f) ("c" ++ show n) x jType ctempCastTyp),
...
transSA this super = TS {toTS = (up (transS this super)) {
   ...
   genApply = \f _ x jType ctempCastTyp ->
      do (tailPosition :: Bool) <- ask
         ...
         if tailPosition
           then nextApply (up this) (left $ var f) x jType
           else (whileApply (up this) False (left $ var f) ("c" ++ show n) x jType ctempCastTyp)
  }}
