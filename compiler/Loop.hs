...
loop :: Connection -> CompileOpt -> Mod -> ValueContext -> Env.Env -> Hist.Hist -> Hist.Hist -> Int -> Bool -> Bool -> Bool -> Bool -> Int -> InputT IO ()
loop handle opt mod val_ctx env hist histOld index flagC flagH flagT flagS num = do
  let max = length hist
  case flagH of
    True  -> case (index == max) of
      True  -> do
        let hist = histOld
        loop handle opt mod val_ctx env hist histOld 0 flagC False flagT flagS num
      False -> do
        ...
        runCMD handle opt mod val_ctx env hist histOld (index+1) flagC True flagT flagS num cmd
    False -> do
      msg <- getInputLine "f2ji> "
      case msg of
        Nothing    -> return ()
        Just ""    -> loop handle opt mod val_ctx env hist histOld 0 flagC False flagT flagS num
        Just input -> do
          case input of
...
runCMD :: Connection -> CompileOpt -> Mod -> ValueContext -> Env.Env -> Hist.Hist -> Hist.Hist -> Int -> Bool -> Bool -> Bool -> Bool -> Int -> String -> InputT IO ()
runCMD handle opt mod val_ctx env hist histOld index flagC flagH flagT flagS num msg = do
  case (stripPrefix ":" msg) of
    ...
    Nothing   -> do
      result <- liftIO (checkType val_ctx (mod ++ msg))
      case result of
        Left typeErr          -> do
          outputStrLn (show typeErr)
          loop handle opt mod val_ctx env hist histOld index flagC flagH flagT flagS num
        Right (tchecked, t) -> do
          let filename = "main_" ++ (show num) ++ ".sf"
          let bind = Env.reverseEnv env
          let msgEnv = mod ++ " " ++ (Env.createBindEnv bind) ++ msg
...
processCMD :: Connection -> CompileOpt -> Mod -> ValueContext -> Env.Env -> Hist.Hist -> Hist.Hist -> Int -> Bool -> Bool -> Bool -> Bool -> Int -> [String] -> InputT IO ()
processCMD handle opt mod val_ctx env hist histOld index flagC flagH flagT flagS num (x:xs) = do
  case x of
    ":help" -> do
      liftIO printHelp
      loop handle opt mod val_ctx env hist histOld index flagC flagH flagT flagS num
...
-- #if MIN_VERSION_z3(0,3,2)
      ...
      Just "method" -> do
        ...
        case result of
          Left (_ :: E.SomeException) -> do
            ...
            loop handle opt mod val_ctx env hist histOld index flagC flagH flagT flagS num
          Right optNew                -> do
            loop handle optNew mod val_ctx env hist histOld index flagC flagH flagT flagS num
      Just "simplify" -> do
        let (y:ys) = xs
        case getCMD ys of
    ...
    ":replay" -> case getCMD xs of 
      Nothing -> do
        ...
        loop handle opt mod val_ctx envNew hist histOld 0 flagC True flagT flagS num
      Just "default" -> do
        cmd <- liftIO (readFile "default.txt")
        let histOld = hist
        let hist = lines cmd
        loop handle opt mod val_ctx env hist histOld 0 flagC True flagT flagS num
